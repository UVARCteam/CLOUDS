...
from Modules.ShowMessage import Iteration_trigger, GA_Iter_history
...
class GeneticAlgorithm(object):
    ...
    def execute_ga(self, x0, vlb, vub, vob, bits, pop_size, max_gen, random_state, Pm=None, Pc=0.5):
        ...
        for generation in range(max_gen + 1):
            import time
            start=time.time()
            Iteration_trigger.trigger = generation
            ...
            if min_fit < fopt:
                fopt = min_fit
                GA_Iter_history.history = float(fopt)
                xopt = min_x
            ...
            #print('\nin ga=',xopt, fopt, nfit, 'time!!=',(end-start)/60,' min')
